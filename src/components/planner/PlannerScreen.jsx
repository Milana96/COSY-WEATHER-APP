...
export default function PlannerScreen({ settings }) {
  ...
  const activityNameById = useMemo(() => {
    return Object.fromEntries(
      activityCatalog.map((activity) => [activity.id, activity.name]),
    );
  }, [activityCatalog]);

  const cityNameByLocationId = useMemo(() => {
    return Object.fromEntries(
      savedLocations.map((location) => [location.id, location.cityName]),
    );
  }, [savedLocations]);
        ...
        <section className="hero-card panel">
          <div className="hero-copy">
            ...
            <p className="hero-description">
              Keep the weather story close while you plan activities, packing,
              and outdoor windows.
            </p>
          </div>
          ...
          <div className="hero-pill-group">
            <div className="hero-pill">
              <span>Now</span>
              <strong>{weather ? `${weather.city}` : "Choose a city"}</strong>
            </div>
            <div className="hero-pill">
              <span>Signal</span>
              <strong>{planner ? planner.comfortLabel : "Loading"}</strong>
            </div>
          </div>
        </section>
            ...
            <>
              <section className="planner-card reveal-1">
                ...
                <p className="planner-copy">
                  {planner.comfortLabel} conditions in {weather.city}. Best
                  outdoor slot: {planner.bestHourLabel}
                  ...
                  {planner.dailySignals.map((day) => (
                    <article key={day.dayLabel} className="planner-day-item">
                      <header>
                        ...
                        <span>
                          {day.maxTemp.value}
                          {day.maxTemp.label} / {day.minTemp.value}
                          {day.minTemp.label}
                        </span>
                      </header>
                      <p>{day.signal}</p>
              ...
              <section className="planner-card reveal-4">
                ...
                <p className="planner-copy">
                  Weather right now is <strong>{activeCondition}</strong>, so
                  these ideas are pre-filtered for you.
                </p>
                ...
                <div className="planner-days" style={{ marginTop: 12 }}>
                  {activities.map((activity) => (
                    <article key={activity.id} className="planner-day-item">
                      ...
                      <button
                        type="button"
                        onClick={() => onCreatePlan(activity.id)}
                      >
                        Add to planner
                      </button>
                    </article>
                ...
                {saveMessage ? (
                  <p className="planner-copy" style={{ marginTop: 12 }}>
                    {saveMessage}
                  </p>
                ) : null}
              </section>
              ...
              <section className="planner-card reveal-4">
                ...
                {!plannerEntries.length ? (
                  <p className="planner-copy">
                    No entries yet. Add one from the suggestion cards above.
                  </p>
                ) : (
                  <div className="planner-days">
                    {plannerEntries.slice(0, 6).map((entry) => {
                      const activityName =
                        activityNameById[entry.activityId] ??
                        "Unknown activity";
                      const cityName =
                        cityNameByLocationId[entry.locationId] ??
                        "Unknown city";
                      const plannedLabel = new Date(
                        entry.plannedDate,
                      ).toLocaleDateString();
